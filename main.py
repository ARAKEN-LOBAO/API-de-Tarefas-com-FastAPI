...
#SERVER PARA DEFENIR O FORMATO DOS DADOS
...
async def atualizar_tarefa(tarefa_id: int, tarefa_atualizada: Tarefa):
    for index, t in enumerate(db_tarefas):
        if t.id == tarefa_id:
            tarefa_atualizada.id = tarefa_id   #Garantir que o ID permaneça o mesmo
...
async def deletar_tarefa(tarefa_id: int):
    for  t in  db_tarefas:
        if t.id == tarefa_id:
            db_tarefas.remove(t)
            return
    raise HTTPException(status_code=404, detail="Tarefa Deletada com sucesso")
